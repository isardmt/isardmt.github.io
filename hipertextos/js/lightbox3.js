...
const SHRINK_SPRING = { stiffness: 200, damping: 28, mass: 1 };
...
class Lightbox {
    ...
    open(target, indexOrTrigger) {
        if (target instanceof HTMLElement) {
            this.openTrigger(target.closest(this.opts.selector) || target);
            return;
        }
        if (!(indexOrTrigger instanceof HTMLElement)) {
            const items = document.querySelectorAll(`[data-lightbox="${CSS.escape(target)}"]`);
            if (items.length > 0) {
    ...
    openImage(src, triggerEl) {
        ...
        const fromCenter = thumbRect.width === 0 || thumbRect.height === 0 || !this.isInViewport(thumbRect);
        ...
        this.createChrome();
        this.computeChromeDrift(originRect.x + originRect.width / 2, originRect.y + originRect.height / 2);
    ...
    openTextLinkWithImage(triggerEl, src, natW, natH) {
        ...
        this.animateSpring({
            ...
            crop: 0,
            borderRadius: 0,
        }, { translateX: 0, translateY: 0, scale: 1, opacity: 1, crop: 0, borderRadius: targetBR }, this.opts.springOpen, () => {
            this.state.isAnimating = false;
            this.updateCursorState();
            this.emit('opened');
        }, undefined, undefined, undefined, openVisuallyDone);
    ...
    close() {
        ...
        if (!thumbVisible) {
            ...
            this.animateSpring({ translateX: 0, translateY: 0, scale: 1, opacity: 1, crop: 0, borderRadius: currentBR }, { translateX: 0, translateY: 0, scale: 0, opacity: 0, crop: 0, borderRadius: currentBR }, SHRINK_SPRING, () => this.finishClose(), closeWhenInvisible);
            return;
        }
        ...
        this.animateSpring({ translateX: 0, translateY: 0, scale: 1, opacity: 1, crop: 0, borderRadius: currentBR }, {
            translateX: flipX,
            translateY: flipY,
            scale: flipScale,
            opacity: 0,
            crop: hasCrop ? 1 : 0,
            borderRadius: this.thumbBorderRadius,
        }, this.opts.springClose, () => this.finishClose(), closeWhenInvisible);
    }
    finishClose() {
    ...
    dismissClose(velocityX, velocityY) {
        ...
        if (!thumbRect || !this.isInViewport(thumbRect)) {
            ...
            }, { translateX: 0, translateY: 0, scale: 0, opacity: 0, crop: 0, borderRadius: currentBR }, SHRINK_SPRING, () => this.finishClose(), (s) => s.opacity < 0.01 && s.scale < SHRINK_FADE_END, { translateX: velocityX, translateY: velocityY });
            return;
        }
        ...
        this.animateSpring({
            ...
            crop: hasCrop ? 1 : 0,
            borderRadius: this.thumbBorderRadius,
        }, this.opts.springClose, () => this.finishClose(), atThumbnail, { translateX: velocityX, translateY: velocityY }, dismissConfigs);
    }
    ...
    pinchClose() {
        ...
        if (!thumbRect || !this.isInViewport(thumbRect)) {
            this.shrinkingToCenter = true;
            this.animateSpring({ translateX: panX, translateY: panY, scale, opacity, crop: 0, borderRadius: currentBR }, { translateX: 0, translateY: 0, scale: 0, opacity: 0, crop: 0, borderRadius: currentBR }, SHRINK_SPRING, () => this.finishClose(), (s) => s.opacity < 0.01 && s.scale < SHRINK_FADE_END);
            return;
        }
        ...
        this.animateSpring({ translateX: panX, translateY: panY, scale, opacity, crop: 0, borderRadius: currentBR }, {
            translateX: flipX,
            translateY: flipY,
            scale: flipScale,
            opacity: 0,
            crop: hasCrop ? 1 : 0,
            borderRadius: this.thumbBorderRadius,
        }, this.opts.springClose, () => this.finishClose(), atThumbnail);
    }
    pinchSnapBack() {
        const { panX, panY, scale } = this.zoom;
        const dismissProgress = 1 - scale;
    ...
    maxFitScale() {
        ...
        if (upscale === true)
            return Infinity;
        if (typeof upscale === 'number' && upscale > 1)
            return upscale;
        return 1;
    }
    /**
     ...
     * `maxScale` allows.
     */
    computeTargetRect(naturalWidth, naturalHeight, maxScale = this.maxFitScale()) {
        const vw = window.innerWidth;
        const vh = window.innerHeight;
